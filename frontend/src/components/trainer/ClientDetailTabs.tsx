...
import { ClientAttachmentsPanel } from '@/components/trainer/ClientAttachmentsPanel'
import { ClientInvitePanel } from '@/components/trainer/ClientInvitePanel'
import { ClientNutritionPanel } from '@/components/trainer/ClientNutritionPanel'
import { downloadClientProgressPdf } from '@/features/api/analytics-service'
  ...
  return (
    <Tabs defaultValue={defaultTab}>
      <TabsList className="flex h-auto flex-wrap gap-1">
        ...
        <TabsTrigger value="nutrition">{t('clients.tabs.nutrition')}</TabsTrigger>
        ...
        <TabsTrigger value="invite">{t('clients.tabs.invite')}</TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="overview" className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            {progress.length ? (
              <p className="text-sm">
                {progress[progress.length - 1]!.date}: {progress[progress.length - 1]!.weight}{' '}
                {t('common:units.kg')}
              </p>
            ) : (
        ...
        <Card>
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
            ...
            <Button
              variant="secondary"
              size="sm"
              disabled={pdfLoading || progress.length === 0}
              onClick={async () => {
                setPdfLoading(true)
                try {
                  await downloadClientProgressPdf(id)
      ...
      <TabsContent value="nutrition">
        <ClientNutritionPanel key={client.id} client={client} />
      </TabsContent>
      ...
      <TabsContent value="files">
        <ClientAttachmentsPanel clientId={client.id} />
      </TabsContent>
